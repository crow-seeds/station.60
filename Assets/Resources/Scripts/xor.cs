...
public class xor : twoInputGate
{
    ...
    public override void inputShape(GameObject s, inOut i, int x, int y, bool on, int z, float time)
    {
        ...
        else if (inputs == 1 && on)
        {
            ...
            {
                if ((pixels1[j].a > .8f && pixels2[j].a < .8f) || (pixels1[j].a < .8f && pixels2[j].a > .8f))
                {
                    pixels1[j].a = 1;
                }
                else
            ...
            inputs++;
            i.delayedActivate(x, y, shape, "xor", true, z, time);
        }
        else if (!on)
        {
            i.delayedActivate(x, y, i.offShape, "xor", false, z, time);
            inputs = 0;
        }
    }
}
